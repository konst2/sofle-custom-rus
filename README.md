...
# Sofle Custom RUS — прошивка для QMK с независимым от раскладки ОС расположением символов

Прошивка предназначена для эргономичной split-клавиатуры Sofle RGB v2 (MX) 
на контроллерах RP2040.  
...
Данная прошивка для **проводной** клавиатуты с поддержкой `QMK`.  
**Для беспроводной версии клавиатуры (`ZMK`)** находится тут - [sofle-custom-rus-zmk](https://github.com/konst2/sofle-custom-rus-zmk)
...
[ADJUST Layer](https://www.keyboard-layout-editor.com/#/gists/6d7ea032318fbbd707c3e5155464b0ed)

Системные настройки:   

![ADJUST Layer](soflekeyboard_adjust.png)
...
Клавиша `S` (кастомный код KC_CH_OS) на слое Ajust переключает OS. В зависимости от OS изменяется расположение клавиш `CMD` и `CTRL`, а так же изменяются клавиши для перемещения и удаления из следующего раздела
...
>Уже скомпилированная прошивка для контроллера rp2040 в файле `sofle_rev1_sofle_custom_rus_rp2040_ce.uf2`

Для разработки использована версия QMK 1.1.8  
...
```
По умолчанию QMK устанавливается в каталог `~/qmk_firmware`. Именно для него написаны скрипты.  
...
- компиляция с прошивкой
```bash
./flash.sh
```

прошиваются обе половины клавиатуры  
на стандартной плате контроллер переводится в режим прошивки при быстром нажатии два раза кнопки `reset` на клавиатуре.

# Проблемы  

1. Некоторые энкодеры несколько раз отправляют код клавиши на один щелчок.  
   Решается исправлением в файлах `~/qmk_firmware/keyboards/sofle/keyhive/keyboard.json` и `~/qmk_firmware/keyboards/sofle/rev1/keyboard.json` всех параметров у encoders с
```json
"resolution": 2
```
на другое число. В моём случае
```json
"resolution": 4
```
